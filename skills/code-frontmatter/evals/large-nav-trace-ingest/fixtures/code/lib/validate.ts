/**
 * ---
 * purpose: Input validation for the write path — validateRawEvent turns an untrusted body into a RawEvent (or throws ValidationError with field+code); enforces clock-skew, userId-length, and properties-size limits. No derived fields here.
 * related:
 *   - ../store/store.ts - validates against EVENT_TYPES and returns the RawEvent shape defined there
 *   - ../services/ingestion.ts - sole caller; runs this as step 1 before enrichment
 *   - ../routes/ingest.ts - catches ValidationError and maps its code/field to a 400 body
 *   - ../services/enrichment.ts - the next step; note timestamp checks here are sanity-only, the server timestamp is assigned there
 * ---
 *
