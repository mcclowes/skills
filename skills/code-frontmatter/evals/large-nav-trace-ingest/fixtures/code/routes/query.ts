/**
 * ---
 * purpose: HTTP read endpoints (GET /v1/metrics/count, /unique-users, /top-types) — parse + validate query params (range/type/granularity/limit), call AggregationService, serialise the MetricResult; bad params yield 400.
 * related:
 *   - ../services/aggregation.ts - does the actual metric computation these handlers call
 *   - ../lib/time.ts - parseRange validates from/to; RangeError_ is mapped to a 400 here
 *   - ../store/store.ts - source of EVENT_TYPES and the EventType/BucketGranularity enums validated against
 * ---
 */
