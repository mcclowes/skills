...
/**
 * ---
 * purpose: ClickHouse EventStore for high-volume deployments — speaks the HTTP interface, batches via JSONEachRow, and uses columnar aggregation (toStartOfHour, uniqExact) for metrics.
 * related:
 ...
 *   - ../config.ts - fromConfig() reads config.clickhouseUrl and clickhouseDatabase
 *   - ./store-postgres.ts - the default sibling backend; same contract, SQL dialect
 * ---
 *
