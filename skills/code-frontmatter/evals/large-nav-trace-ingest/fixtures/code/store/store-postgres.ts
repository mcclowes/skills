...
/**
 * ---
 * purpose: Default/production EventStore backed by Postgres — single `events` table, multi-row batch INSERT, and counts pushed down to SQL via date_trunc so raw rows never come back for aggregates.
 * related:
 *   - ./store.ts - implements the EventStore interface and throws its StorageError
 *   - ../lib/time.ts - buildBucketSeries/granularityMs zero-fill and align the bucketed counts
 *   - ../config.ts - fromConfig() reads config.postgresUrl to build the pool
 *   - ./store-clickhouse.ts - sibling backend with the same contract, alternate SQL dialect
 * ---
 *
