/**
 * ---
 * purpose: Read-path service — turns query-route params into EventStore calls and assembles MetricResults; holds no storage logic itself (bucketing/distinct pushed down to the store), and fans out per-type counts for the leaderboard.
 * related:
 *   - ../store/store.ts - calls countByType/distinctUsers and returns the MetricResult shape defined there
 *   - ../routes/query.ts - HTTP handlers that parse params and call count/uniqueUsers/topEventTypes
 *   - ./ingestion.ts - the write-path counterpart against the same EventStore
 * ---
 */
import {
