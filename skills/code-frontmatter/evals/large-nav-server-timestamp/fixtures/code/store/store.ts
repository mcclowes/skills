/**
 * ---
 * purpose: Storage seam and domain types — defines the EventStore interface (the storage contract), the RawEvent/Event/MetricResult shapes, and StorageError; start here to understand the data model.
 * related:
 *   - ./store-postgres.ts - default EventStore implementation (Postgres)
 *   - ./store-clickhouse.ts - EventStore implementation for high-volume (ClickHouse)
 *   - ./store-memory.ts - in-memory EventStore for tests/dev
 *   - ../server.ts - selects and constructs one of the three implementations at startup
 *   - ../services/enrichment.ts - produces the enriched Event that EventStore.insert stores
 *   - ../lib/validate.ts - validates untrusted input into a RawEvent using EVENT_TYPES from here
 * ---
 *
