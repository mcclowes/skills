/**
 * ---
 * purpose: Per-API-key token-bucket rate limiter (lazy refill, no timer) plus its Express adapter — 429 + Retry-After when a key's bucket is empty; fails closed with 500 if it somehow runs before auth.
 * related:
 *   - ./auth.ts - must run first; this keys buckets on the req.auth.apiKeyId it sets
 *   - ../config.ts - fromConfig() reads config.rateLimit.ratePerSecond and burst
 *   - ../server.ts - mounts the adapter after auth and runs evictIdle on a timer to bound memory
 * ---
 *
