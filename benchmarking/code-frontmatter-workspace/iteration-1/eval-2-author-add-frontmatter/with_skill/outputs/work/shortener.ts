/**
 * ---
 * purpose: Core service - validates a URL, mints a slug, persists it, and resolves slugs (counting clicks).
 * related:
 *   - ./store.ts - LinkStore interface it persists through
 *   - ./codec.ts - turns the store ID into a base62 slug
 *   - ./validate.ts - URL validation/SSRF guard
 *   - ./config.ts - baseUrl for the returned short URL
 * ---
 */
