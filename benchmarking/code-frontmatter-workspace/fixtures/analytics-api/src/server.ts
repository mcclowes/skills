/**
 * ---
 * purpose: App composition root and the SOLE backend selector — maps config.storageBackend to a concrete EventStore, then wires store, services, middleware, and routers in order.
 * related:
 *   - ./config.ts - supplies Config (storageBackend, port, apiKeys, rateLimit) that drives all wiring here
 *   - ./store/store.ts - the EventStore interface every backend implements and everything downstream depends on
 *   - ./store/store-postgres.ts - default backend, constructed when storageBackend=postgres
 *   - ./store/store-clickhouse.ts - backend constructed when storageBackend=clickhouse
 *   - ./store/store-memory.ts - backend constructed when storageBackend=memory
 *   - ./services/ingestion.ts - write-path service mounted under ingestRouter
 *   - ./services/aggregation.ts - read-path service mounted under queryRouter
 *   - ./middleware/auth.ts - auth wall; mounted after healthRouter, before the rate limiter and app routes
 *   - ./middleware/rateLimit.ts - per-key limiter; mounted after auth, plus the evictIdle timer set up here
 * ---
 */
