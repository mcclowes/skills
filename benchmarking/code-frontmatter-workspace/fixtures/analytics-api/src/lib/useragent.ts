/**
 * ---
 * purpose: Dependency-free User-Agent parser — classifies browser/os/device by ordered regex rules (specific before generic) and never returns null, so enrichment gets a complete UserAgentInfo. Open to add/fix a UA family.
 * related:
 *   - ../store/store.ts - returns the UserAgentInfo shape (with its device enum) defined there
 *   - ../services/enrichment.ts - sole caller; folds the parsed result into the stored Event
 * ---
 */
