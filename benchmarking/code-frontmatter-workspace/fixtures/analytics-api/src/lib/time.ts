/**
 * ---
 * purpose: UTC time-bucketing toolkit shared by every store and the query routes — floors timestamps to hour/day, parses from/to query pairs into a validated TimeRange, and zero-fills dense bucket series.
 * related:
 *   - ../store/store.ts - defines TimeRange/TimeBucket/BucketGranularity used throughout here
 *   - ../routes/query.ts - calls parseRange and surfaces RangeError_ as a 400
 *   - ../store/store-postgres.ts - uses buildBucketSeries/granularityMs to align SQL-aggregated counts
 *   - ../store/store-clickhouse.ts - same bucket alignment for ClickHouse-aggregated counts
 *   - ../store/store-memory.ts - uses floorTo/isWithin/buildBucketSeries to bucket in JS
 * ---
 */
